...
class Grid(object):
    ...
    def resolveUri(self, uriref):
        ...
        resolveduri  = urljoin(self._baseuri, uriref)
...
class GridCSV(Grid):
    ...
    @param csvfile:     File object that returns CSV data
...
class GridTSV(Grid):
    ...
    @param tsvfile:     File object that returns tab-separated data
    @param baseuri:     A string used as the base URI for references in the grid.
    @param dialect:     An optional dialect parameter (e.g. 'excel', 'excel-tab').
    ...
    def __init__(self, tsvfile, baseuri=None, dialect=None):
...
class GridExcel(Grid):
    ...
    def __init__(self, xlsfilename, baseuri=None):
        if not xlrd:
            raise ValueError("Can't read Excel sheet: xlrd not available")
